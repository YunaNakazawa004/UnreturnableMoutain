...
CParticle3D* CParticle3D::Create(const D3DXVECTOR3 posP, const int nLifeP, const int nValue,
	...
	const bool bHoming, CObject* HomingObj, D3DXVECTOR3 HomingPos, const float fSpeedHoming)
{
#ifndef LIST
	...
	{// NULLチェック
		...
		if (FAILED(pParticle3D->Init(posP, nLifeP, nValue, fRadius, fAddRadius, fMinusAlpha, typeE, typeP, nLifeE, 
			fSpeedE, bSpeedEInertia, col, fDist, bHoming, HomingObj, HomingPos, fSpeedHoming)))
		{// もし失敗した場合
	...
	{// 生成するだけ繰り返す
		...
		{// NULLチェック
			...
			case TYPE_PIN:			// 光柱
				...
				rot.x = (float)(rand() % 300 - 100) / 1000.0f;		// 角度を設定
				rot.y = 1.0f;		// 角度を設定
				rot.z = (float)(rand() % 300 - 100) / 1000.0f;		// 角度を設定
			...
			m_apEffect[nCntAppear] = CEffect3D::Create(pos, rot, m_fSpeedE, m_bSpeedEInertia, m_nLifeE, m_fRadius, 
				m_fAddRadius, m_fMinusAlpha, m_typeE, m_col, m_bHoming, m_HomingObj, m_HomingPos, m_fSpeedHoming);
		}
	}
