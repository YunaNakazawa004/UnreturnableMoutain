	...
	for (int nCntPri = 0; nCntPri < MAX_PRIORITY_NUM; nCntPri++)
	{
		...
		while (pObj != NULL)
		{
			...
			{// 木オブジェクトなら当たり判定する
				...
				{// 木と重なった
					// 当たり判定
					bColl = dynamic_cast<CTree*>(pObj)->m_apModel[0]->Collision(posObj, dynamic_cast<CTree*>(pObj)->GetRotation(), dynamic_cast<CTree*>(pObj)->GetScale(), pPos, posOld, move, fRadius, fHeight);
